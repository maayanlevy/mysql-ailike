---
name: mysql-ailike
description: Install, query, and validate the AILIKE MySQL plugin when adding natural-language row filters, integrating it into application SQL, or troubleshooting its syntax and configuration.
license: GPL-2.0-only
---

# AILIKE for MySQL

AILIKE evaluates a natural-language condition against a text value using TypeSafe
Jev. Use it for semantic judgments; keep exact identifiers, arithmetic, date
comparisons, and ordinary joins in SQL. This skill describes the current
two-argument plugin interface.
...
- `ailike_udf.so` provides `ailike(text, prompt)`. `ailike_rewrite.so` enables infix
  syntax by rewriting it before MySQL parses the statement. It is not a SQL
  function for callers to invoke. Function-only installation is supported.
...
by another column. Both arguments must be strings; cast non-string values with
`CAST(... AS CHAR)`.
...
comments (`/*! ... */`). Do not generate `column AILIKE other_column`,
`column AILIKE ?`, or `'literal' AILIKE 'condition'`.

## Design the condition

- Ask one explicit yes/no condition. Jev sees the supplied value and prompt, not
  the rest of the row or database. Include needed context in the value with
  `CONCAT(...)`, or construct the prompt from the relevant columns.
...
- The result is `1` when the model's probability is at least `0.5`, otherwise `0`.
  ...
  There is no `ailike_score`, third argument, or configurable threshold in the
  current interface; do not invent one. Raising a threshold would not resolve an
  ambiguous condition or recover a missed match.

## Bound execution and handle errors

Each uncached value/prompt pair makes a synchronous API request. Narrow candidate
...
The cache is bounded and local to each expression, not shared across queries.
...
these limits. Values are limited to 32 KiB and prompts to 8 KiB; empty prompts
